...
MUSICS_FILE = "music_album_file.json"
GENRES_FILE = "genre_file.json"
...
if File.exist?(MUSICS_FILE) && !File.zero?(MUSICS_FILE)
    musics_data = JSON.parse(File.read(MUSICS_FILE))
  else
    musics_data = []
  end
  
  if File.exist?(GENRES_FILE) && !File.zero?(GENRES_FILE)
...
loop do
    ...
    when 3
      ...
      if genre_index < 0 || genre_index >= genres.length
